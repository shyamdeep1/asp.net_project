    ...
    public partial class manage_job : System.Web.UI.Page
    {
        ...
        private void LoadTodayStats()
        {
            ...
            try
            {
                ...
                // Jobs approved today (assuming there's a status update tracking)
                cmd = new SqlCommand(@"SELECT COUNT(*) FROM Jobs 
                ...
                // Average time to approve (placeholder calculation)
                lblAvgTimeToApprove.Text = "2.5 hrs";
            }
            catch (Exception ex)
            {
                // Set default values
                lblTodayPosted.Text = "0";
                lblTodayApplications.Text = "0";
                lblTodayApproved.Text = "0";
                lblAvgSalary.Text = "$0";
                lblWeeklyApprovalRate.Text = "0%";
                lblAvgTimeToApprove.Text = "0 hrs";
            }
            finally
            {
                con.Close();
        ...
        private void LoadJobsGrid()
        {
            ...
            {
                // Handle error
                lblResultsInfo.Text = "Error loading jobs: " + ex.Message;
            }
            finally
            {
                con.Close();
        ...
        private void LoadRecentActivities()
        {
            ...
            {
                // Handle error silently for recent activities
            }
            finally
            {
                con.Close();
        ...
        private void UpdateJobStatus(int jobId, string status)
        {
            ...
            {
                ClientScript.RegisterStartupScript(this.GetType(), "alert", 
                    $"alert('Error updating job status: {ex.Message}');", true);
            }
            finally
            {
                con.Close();
